...
namespace JPB.DataAccess.Manager
{
    partial class DbAccessLayer
    {
        ...
        {
            var query = CreateDeleteQueryFactory(this.GetClassInfo(typeof(T)), null);
        ...
        {
            var query = CreateDeleteQueryFactory(this.GetClassInfo(entry.GetType()), entry);
        ...
        {
            var query = CreateDeleteQueryFactory(this.GetClassInfo(entry.GetType()), entry, parameter);
            RaiseDelete(entry, query);
            Database.PrepaireRemoteExecution(query);
            Database.Run(s => { s.ExecuteNonQuery(query); });
        }
