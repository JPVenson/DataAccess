		...
		public virtual IPropertyInfoCache<TAtt> Init(PropertyInfo propertyInfo, bool anon)
		{
			...
			if (!anon && (getMethod != null || setMethod != null))
			{
				...
				if (isStatic)
				{
					...
					if (getMethod != null)
					{
						var getterDelegate = Expression.Lambda(accessField);
					...
					if (setMethod != null)
					{
						...
						var setterDelegate = Expression.Lambda(setter, valueRef);
				...
				else
				{
					...
					if (getMethod != null)
					{
						var getExpression = Expression.Lambda(accessField, thisRef);
