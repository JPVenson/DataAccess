...
#endif
{
	...
	public class LocalDbWithConstraintsTest
	{
		...
		{
			var images = TestInit(null, new[] { new LocalDbUniqueConstraint<Image, int>("BookId is Unique", s => s.IdBook) }, null);
		...
		public void AddUniqueConstraintAfter()
		{
			...
			Assert.That(() =>
			{
				images.Constraints.Unique.Add(new LocalDbUniqueConstraint<Image, int>("BookId is Unique", s => s.IdBook));
			}, Throws.Exception.TypeOf<InvalidOperationException>());
		}
		...
		public void AddDefaultConstraint()
		{
			...
			{
				new LocalDbDefaultConstraint<Image, int>("DefaultConstraint", 666, (source, constVal) => source.IdBook = constVal)
			});
		...
		public void AddDefaultConditionalConstraint()
		{
			var images = TestInit(null, null, new[]
			{
				new LocalDbDefaultConstraintEx<Image, int>(new DbConfig(), "DefaultConstraint", () => 666, source => source.IdBook)
			});
